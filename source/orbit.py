...
class Orbit(Physics):
    ...
    def eccentricity(self, velocity, relative, angular_momentum, k):
        e = (np.cross(velocity, angular_momentum) / k) - (relative / self.mag(relative))
        return e

    def inclination(self, angular_momentum):
        q = angular_momentum[2] / self.mag(angular_momentum)
        return np.arccos(q)

    def node_line(self, angular_momentum):
    ...
    def ascention_node(self, N):
        if self.mag(N) == 0:
            ...
            Omega = np.arccos(N[0] / self.mag(N))
        elif N[1] < 0:
            Omega = 2 * np.pi - np.arccos(N[0] / self.mag(N))
        
        return Omega

    def periapsis(self, ascention_node, eccentricity):
        Necc = np.dot(ascention_node, eccentricity)
        ne = self.mag(ascention_node) * self.mag(eccentricity)
    ...
    def true_anomaly(self, eccentricity, relative, radial_velocity):
        ...
        er = self.mag(eccentricity) * self.mag(relative)
